...
ship = []
ship2 = []
nbOfStacks = len(Lines[0]) / 4
for i in range( int(nbOfStacks)):
    stack = []
    ship.append(stack)


for line in Lines:
    if (line == '\n' or line[1] == '1'):                          # fin du schéma de base
    ...
    # Moove the supplies
        ...
        for j in range(nbToMove):
            stackMoved = ship[fromStack].pop()
            ship[toStack].append(stackMoved)
            
            stackMoved2 = ship2[fromStack].pop(-nbToMove + j)
            ship2[toStack].append(stackMoved2)
    ...
    # Fill the stacks
        ...
        for i in range(1, len(Lines[0]), 4):
            ...
            stack = i // 4
            if (supplie != ' '):
                ship[stack].insert(0, supplie)


result1 = []
result2 = []
for i in range( int(nbOfStacks)):
    result1.append(ship[i].pop())
    result2.append(ship2[i].pop())
print(result1)
print(result2)