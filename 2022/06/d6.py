...
the fourth character is received, making the most recent four characters mjqj. Because j 
...
subroutine should report the value 7, because the first start-of-packet marker is complete 
...
for i in range(length - (MarkerLength - 1)):
    for j in range(MarkerLength):
        markerSet.add(line[i + j])
...
for i in range(length - (MessageMarkerLength - 1)):
    for j in range(MessageMarkerLength):
        messageSet.add(line[i + j])
        
    if(len(messageSet) == MessageMarkerLength):
        messageStart = i + MessageMarkerLength
        break
        
    else: 
        messageSet.clear()
    
print(markerStart, messageStart)