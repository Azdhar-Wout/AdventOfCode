...
have item type B somewhere in their rucksack, and at most two of the Elves will be carrying any other 
item type.
...
for backpack in Lines:
    ...
    for x in range(length-1):
        if(groupMember == 0):
            setResult2.add(backpack[x])
        
        if(x < middle):
            pouch1.add(backpack[x])
        else:
            pouch2.add(backpack[x])
    
    
    itemInBothPoutch = pouch1.intersection(pouch2)
    pouch1.update(pouch2)
    setResult2 = setResult2.intersection(pouch1)
    
    for item1 in itemInBothPoutch:
        if (ord(item1) >= 97):
            result1 += ord(item1) - 96
        else:
            result1 += ord(item1) - 38
            
    
    if(groupMember == 2):
        for item2 in setResult2:
            if (ord(item2) >= 97):
                result2 += ord(item2) - 96
            else:
                result2 += ord(item2) - 38
    groupMember += 1
